"""Qlever + Photon + NCES facility finder — v6 (+ JSON export).

Same data pipeline as qlever_v_6.py (Qlever SPARQL + NCES CCD schools,
untouched). Adds two JSON outputs per city, alongside the existing Excel
workbook:
  - `<city>_raw_response.json` — the raw JSON payloads as returned by the
    Qlever SPARQL endpoint and the NCES CCD API for that city.
  - `<city>_facilities.json` — one entry per physical facility with every
    sport/field grouped under it (e.g. a park with both a basketball court
    and a soccer field appears once, with both sports nested under it).
Also strips functions/classes that were defined but never called anywhere
in the pipeline.
...
def query_qlever(name, query, status_callback, use_cache=True, timeout=90):
    ...
    raw_payload is the SPARQL JSON response exactly as Qlever returned it
    (for the raw-response JSON export). On a cache hit the full SPARQL
    payload isn't retained (only the flattened rows are cached), so
    raw_payload is a small marker dict instead.
    """
    if use_cache:
...
def fetch_overpass(bbox, sport_config, overpass_url, status_callback,
    ...
    Returns (results, raw_payloads) — raw_payloads is the raw SPARQL JSON
    response per sub-query name, exactly as returned by Qlever.
    """
...
def fetch_nces_schools(target_city, target_state, bbox, status_callback,
    ...
    Each matched entry also carries the original, unmodified NCES record
    under "_nces_raw" so the raw-response JSON export can surface it
    without changing this function's return signature or any call site.
    """
...
def _run_single_job(city, county, state, sport_choice, overpass_url, use_cache,
    ...
    try:
        ...
        op_results, raw_qlever = fetch_overpass(bbox, sport_config, overpass_url, _log,
        ...
        nces_results = fetch_nces_schools(city, state, bbox, _log,
                                            use_cache=use_cache)
        raw_response = {
            "qlever_queries": raw_qlever,
            "nces_matched_raw": [e["_nces_raw"] for e in nces_results if "_nces_raw" in e],
        }
        ...
        if not merged:
            ...
            return sport_choice, {}, 0, raw_response, None
        ...
        return sport_choice, categories, total, raw_response, None
    except Exception as e:
...
def run_batch(rows, sports_selected, overpass_url, use_cache, max_workers,
    ...
    raw_results = {r["city"]: {} for r in rows}
    errors = []
    done = 0
    t0 = time.time()
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            try:
                sp, cats, total, raw_response, err = fut.result()
                if err:
                    errors.append(err)
                if cats is not None:
                    results[city][sp] = (cats, total)
                raw_results[city][sp] = raw_response
            except Exception as e:
    ...
    return results, raw_results, errors

def build_facility_summary_json(city, sport_results):
    """Group every facility across all fetched sports into one entry per
    physical facility, listing every field/court it has (e.g. a park with
    both a basketball court and a soccer field appears once, with both
    sports nested under it).

    Grouping key is the PARENT facility's own lat/lon (entry["lat"]/["lon"]),
    not each individual field's lat/lon. A facility with multiple pitches
    (e.g. a park with 3 soccer fields) has each pitch at a distinct
    coordinate — keying on the per-field coordinate would splinter one real
    facility into several near-duplicate JSON entries.
    """
    facilities = {}
        ...
        for category_name, entries in categories.items():
            for entry in entries:
                key = (normalize_key(entry["name"]),
                       round(entry["lat"], 4), round(entry["lon"], 4))
                fac = facilities.setdefault(key, {
                    "name": entry["name"],
                    "address": entry.get("address", ""),
                    "lat": entry["lat"],
                    "lon": entry["lon"],
                    ...
                    "zipcode": entry.get("zipcode")
                               or _parse_zip_from_address(entry.get("address", "")),
                    "google_earth_url": (
                        f"https://earth.google.com/web/@{entry['lat']},"
                        f"{entry['lon']},50a,300d,35y,0h,0t,0r"
                    ),
                    "sports": {},
                })
                rows = expand_to_rows([entry], sport_config, category_name, sport_choice)
                for row in rows:
                    fac["sports"].setdefault(sport_choice, []).append({
                        "description": row["description"],
                        "length_1_ft": row["length_1_ft"],
                        "width_1_ft": row["width_1_ft"],
                        "length_2_ft": row["length_2_ft"],
                        "width_2_ft": row["width_2_ft"],
...
def build_raw_response_json(city, raw_results_for_city):
    """Raw JSON payloads as returned by the Qlever SPARQL endpoint and the
    NCES CCD API, keyed by sport, for one city."""
    return {"city": city, "responses": raw_results_for_city}

def _build_zip(per_city_results, raw_results, status_callback):
    buf = io.BytesIO()
    grand_total = 0
    cities_with_data = 0
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for city, sport_results in per_city_results.items():
            ...
            facility_summary = build_facility_summary_json(city, sport_results)
            zf.writestr(f"{safe_name}_facilities.json",
                        json.dumps(facility_summary, indent=2))

            raw_json = build_raw_response_json(city, raw_results.get(city, {}))
            zf.writestr(f"{safe_name}_raw_response.json",
...
def main():
    ...
    Volleyball). Also produces the raw Qlever SPARQL + NCES API JSON
    responses and a combined per-facility JSON (every sport/field grouped
    under its facility). Runs all jobs in parallel.
    """)
    ...
    if run_single:
        if not single_city.strip() or not single_state.strip():
            ...
            with status:
                t0 = time.time()
                per_city_results, raw_results, errors = run_batch(
            ...
            # Cache in session_state: st.download_button triggers a full
            # rerun, and a plain st.button resets to False on that rerun —
            # without caching, the whole results block (and its other
            # download buttons) would vanish after the first click.
            st.session_state["single_result"] = {
                "row": row,
                "sport_results": sport_results,
                "raw_results": raw_results,
                "wb_bytes": wb_buf.getvalue() if wb_buf else None,
                "n": n,
                "errors": errors,
    ...
    if single_result:
        row = single_result["row"]
        sport_results = single_result["sport_results"]
        n = single_result["n"]
        st.subheader(f"🏟️ {row['city']}, {row['state']}")
        if st.button("🔄 Clear result / start a new search", key="clear_single"):
            del st.session_state["single_result"]
            st.rerun()
        if single_result["wb_bytes"] is None or n == 0:
            ...
            facility_summary = build_facility_summary_json(row["city"], sport_results)
            st.download_button(
                label=f"📥 Download {safe_name}_facilities.json "
                      f"({facility_summary['facility_count']} facilities, all sports grouped)",
                data=json.dumps(facility_summary, indent=2),
                file_name=f"{safe_name}_facilities.json",
                mime="application/json",
                use_container_width=True,
                key="dl_single_facilities",
            )

            raw_json = build_raw_response_json(
                row["city"], single_result["raw_results"].get(row["city"], {}))
            st.download_button(
                label=f"📥 Download {safe_name}_raw_response.json "
                      f"(raw Qlever SPARQL + NCES API responses)",
                data=json.dumps(raw_json, indent=2),
                file_name=f"{safe_name}_raw_response.json",
                mime="application/json",
                use_container_width=True,
                key="dl_single_raw",
            )
        if single_result["errors"]:
            with st.expander(f"⚠️ {len(single_result['errors'])} error(s)"):
                for e in single_result["errors"]:
                    st.text(e)
        with st.expander(f"📜 Log ({len(single_result['log_messages'])} lines)"):
            st.code("\n".join(single_result["log_messages"]), language="text")
        return
    ...
    if st.button("🚀 Run Batch", type="primary", use_container_width=True):
        ...
        with status:
            t0 = time.time()
            per_city_results, raw_results, errors = run_batch(rows, sports_selected,
                                                                overpass_url, use_cache,
                                                                max_workers, log)
            st.write(f"All jobs done in {time.time()-t0:.1f}s. Building workbooks...")
            zip_buf, cities_done, grand_total = _build_zip(per_city_results, raw_results, log)
            status.update(label=f"✅ Done — {cities_done} cities, "
                                  f"{grand_total} facility rows",
                            state="complete")

        # Cache in session_state so the download button below doesn't wipe
        # these results on the rerun it triggers (same reason as single-city).
        st.session_state["batch_result"] = {
            "zip_bytes": zip_buf.getvalue(),
            "zip_file_name": f"facilities_batch_{int(time.time())}.zip",
    ...
    if batch_result:
        st.success(f"✅ Built {batch_result['cities_done']} workbooks with "
                   f"{batch_result['grand_total']} total rows")

        if batch_result["errors"]:
            with st.expander(f"⚠️ {len(batch_result['errors'])} job error(s)"):
                for e in batch_result["errors"]:
                    st.text(e)

        with st.expander(f"📜 Full log ({len(batch_result['log_messages'])} lines)"):
            st.code("\n".join(batch_result["log_messages"]), language="text")

        st.caption("Each city in the zip gets 3 files: `<city>.xlsx`, "
                   "`<city>_facilities.json` (all sports grouped per facility), "
                   "and `<city>_raw_response.json` (raw Qlever + NCES responses).")
        st.download_button(
            label="📥 Download results.zip",
            data=batch_result["zip_bytes"],
            file_name=batch_result["zip_file_name"],
            mime="application/zip",
            type="primary",
            use_container_width=True,
            key="dl_batch_zip",
        )
        if st.button("🔄 Clear result / run another batch", key="clear_batch"):
