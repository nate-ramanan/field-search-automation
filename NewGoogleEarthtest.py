...
def getImage(lat, lon, key, zoom, width, height):
    if pd.isna(lat) or pd.isna(lon) or lat == 'Error' or lon == 'Error':
        return 'Error'
...
def get_satellite_image_array(gps_location, zoom_level=18, size=(800, 850)):
    if not gps_location or pd.isna(gps_location.get('latitude')) or pd.isna(gps_location.get('longitude')):
        return None
...
def parse_osm_elements(osm_data, search_sport_type, city, state, postal_code):
    ...
    for el in osm_data.get("elements", []):
        ...
        if not lat or not lon or pd.isna(lat) or pd.isna(lon): continue
...
def parse_google_elements(google_data, search_sport_type, city, state, postal_code):
    ...
    for res in google_data.get("results", []):
        ...
        field = {
            ...
            'postal_code': postal_code,
            'street': res.get("formatted_address", "").split(",")[0] if res.get("formatted_address") else "", 
            'city': city,
            'state': state,
            'original_gps_location': {'latitude': float(lat), 'longitude': float(lon)},
            'gplace_id': place_id, 
...
# ---------------------------------------------------------
# DATABASE DEDUPLICATION & METRIC FUNCTIONS
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# IMPLEMENTING PIPELINE CODE BLOCKS
# ---------------------------------------------------------
def save_field_data(fields):
    ...
    try:
        ...
        for f in fields:
            ...
            cur.execute("""
                SELECT 1 FROM public.new_google_earth 
                WHERE gps_location = %s AND search_sport_type = %s LIMIT 1;
            """, (gps_str, f['search_sport_type']))
            ...
            """, (
                f['field_name'], f['formatted_address'], f['postal_code'], 
                f['street'], f['city'], f['state'], gps_str, f['gearth_link'], f['search_sport_type']
            ))
        conn.commit()
    except Exception:
        pass
...
def save_object_data(nge_objects):
    ...
    try:
        ...
        for obj in nge_objects:
            ...
            cur.execute("""
                SELECT 1 FROM public.nge_object 
                WHERE field_search_id = %s AND detected_sport = %s AND adjusted_gps_location = %s LIMIT 1;
            """, (obj['field_search_id'], obj['sport_name'], obj['adjusted_gps']))
...
if __name__ == "__main__":
    ...
    for sport_id, sport_name in SPORT_TAGS.items():
        if source_choice in ['1', '3'] and bbox:
            print(f"[Status] Querying Overpass API for {sport_name} facilities...")
        ...
        if source_choice in ['2', '3']:
            print(f"[Status] Querying Google Places API for {sport_name} facilities...")
    ...
    if all_fields:
        print("[Status] Writing raw coordinates to new_google_earth...")
        save_field_data(all_fields)
                
    print("[Status] Filtering out overlapping spatial duplicates...")
    delete_duplicates()
    
    print("[Status] Fetching target records for YOLO object detection alignment...")
    df = get_field_data()
    nge_object = []
    all_target_class_ids = [4, 5, 9, 10, 14, 16]
    
    total_rows = len(df)
    print(f"[Status] Beginning computer vision adjustment loop for {total_rows} locations...")
    
    for index, row in df.iterrows():
        gps_val = row.get('gps_location')
        
        if pd.isna(gps_val) or not isinstance(gps_val, str) or ',' not in gps_val:
            continue
            
        try:
            lat, lon = gps_val.split(",")
        ...
        # Quick progress tick every 5 fields so you know it's moving
        if (index + 1) % 5 == 0 or (index + 1) == total_rows:
            print(f"   Progress: Checked {index + 1}/{total_rows} elements.")
    
    print("[Status] Saving fine-tuned YOLO detections to nge_object...")
    save_object_data(nge_object)
    print("[Success] Script execution completed cleanly.")